...
public class WaddleTrigger : MonoBehaviour
{
	...
	void LateUpdate()
	{
		...
		if(_needsUpdate)
		{
			...
			Vector3 potentialPos = _positionTransform.transform.position + _rotationTransform.transform.forward * _speed * fixedTimeScale; 
			Vector3 startFromOffset = _rotationTransform.transform.forward * 0.3f;
			Vector3 checkPos = _centerEye.transform.position + startFromOffset + _rotationTransform.transform.forward * _speed * fixedTimeScale; 
